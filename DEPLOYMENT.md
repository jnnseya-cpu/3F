...
- **Dummy data: removed.** `lib/mockData.ts` is **deleted**. The six demo dashboards (`/dashboard`, `/dashboard/provincial`, `/dashboard/local`, `/ethics`, `/infrastructure`, `/policy`) now render an honest **pre-launch empty state** (`components/PreLaunchState.tsx`) — no fabricated members, numbers or activity — while keeping their real features (the AI agent panels, the report/generate forms, and real geographic data on the provincial view). The main dashboard reuses the **real** `PublicLedger` (real aggregates or honest pre-launch state). Fabricated named individuals are gone; `/candidates` keeps clearly-illustrative "Profil exemple" profiles solely to demonstrate the scoring mechanism (not real people). Verified: no dummy strings in the rendered HTML of any of the six pages.
